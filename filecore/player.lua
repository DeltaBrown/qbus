...
QBCore.Player.CheckPlayerData = function(source, PlayerData)
	...
	PlayerData.charinfo.account = PlayerData.charinfo.account ~= nil and PlayerData.charinfo.account or "NL0"..math.random(1,9).."BPRP"..math.random(1111,9999)..math.random(1111,9999)..math.random(11,99)
	...
	PlayerData.metadata["tacorep"] = PlayerData.metadata["tacorep"] ~= nil and PlayerData.metadata["tacorep"] or 0
	...
	PlayerData.gang.grade.name = PlayerData.gang.grade.name ~= nil and PlayerData.gang.grade.name or nil
	...
	PlayerData.position = PlayerData.position ~= nil and PlayerData.position or BPConfig.DefaultSpawn
...
QBCore.Player.CreatePlayer = function(PlayerData)
	...
	self.Functions.SetJob = function(job, grade)
		...
		if QBCore.Shared.Jobs[job] ~= nil then
			...
			if QBCore.Shared.Jobs[job].grades[grade] then
				...
				self.PlayerData.job.payment = jobgrade.payment ~= nil and jobgrade.payment or 10
				self.PlayerData.job.isboss = jobgrade.isboss ~= nil and jobgrade.isboss or false
			else
				self.PlayerData.job.grade = {}
				self.PlayerData.job.grade.name = 'No Grades'
				self.PlayerData.job.grade.level = 0
				self.PlayerData.job.payment = 100
	...
	self.Functions.SetGang = function(gang, level)
	local gang = gang:lower()
	local level = tostring(level) ~= nil and tostring(level) or '0'
	local grades = tonumber(level)

	if QBCore.Shared.Gangs[gang] ~= nil then
		self.PlayerData.gang.name = gang
		self.PlayerData.gang.label = QBCore.Shared.Gangs[gang].label
		self.PlayerData.gang.grades = QBCore.Shared.Gangs[gang].maxLevel
		self.PlayerData.gang.grades = QBCore.Shared.Gangs[gang].grades
		self.PlayerData.gang.level = grades
		
		if QBCore.Shared.Gangs[gang].grades[level] then
			local ganggrade = QBCore.Shared.Gangs[gang].grades[level]
			self.PlayerData.gang.grade = {}
			self.PlayerData.gang.grade.name = ganggrade.name
			self.PlayerData.gang.grade.level = tonumber(level)
	...
	self.Functions.AddMoney = function(moneytype, amount, reason)
		...
		if self.PlayerData.money[moneytype] ~= nil then
			...
			if amount > 100000 then
				TriggerEvent("qb-log:server:CreateLog", "playermoney", "AddMoney", "lightgreen", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** $"..amount .. " ("..moneytype..") there, new "..moneytype.." balance: "..self.PlayerData.money[moneytype], true)
			else
				TriggerEvent("qb-log:server:CreateLog", "playermoney", "AddMoney", "lightgreen", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** $"..amount .. " ("..moneytype..") there, new "..moneytype.." balance: "..self.PlayerData.money[moneytype])
			end
	...
	self.Functions.RemoveMoney = function(moneytype, amount, reason)
		...
		if self.PlayerData.money[moneytype] ~= nil then
			...
			if amount > 100000 then
				TriggerEvent("qb-log:server:CreateLog", "playermoney", "RemoveMoney", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** $"..amount .. " ("..moneytype..") off, new "..moneytype.." balance: "..self.PlayerData.money[moneytype], true)
			else
				TriggerEvent("qb-log:server:CreateLog", "playermoney", "RemoveMoney", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** $"..amount .. " ("..moneytype..") off, new "..moneytype.." balance: "..self.PlayerData.money[moneytype])
			end
	...
	self.Functions.SetMoney = function(moneytype, amount, reason)
		...
		if self.PlayerData.money[moneytype] ~= nil then
			...
			TriggerEvent("qb-log:server:CreateLog", "playermoney", "SetMoney", "green", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** $"..amount .. " ("..moneytype..") new "..moneytype.." balans: "..self.PlayerData.money[moneytype])
			return true
		end
		return false
	end
	...
	self.Functions.AddItem = function(item, amount, slot, info)
		...
		if (totalWeight + (itemInfo["weight"] * amount)) <= QBCore.Config.Player.MaxWeight then
			if (slot ~= nil and self.PlayerData.items[slot] ~= nil) and (self.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo["type"] == "item" and not itemInfo["unique"]) then
				...
				TriggerEvent("qb-log:server:CreateLog", "playerinventory", "AddItem", "green", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** get item: [slot:" ..slot.."], itemname: " .. self.PlayerData.items[slot].name .. ", added amount: " .. amount ..", new total amount: ".. self.PlayerData.items[slot].amount)
			...
			elseif (itemInfo["unique"]) or (not slot or slot == nil) or (itemInfo["type"] == "weapon") then
				for i = 1, BPConfig.Player.MaxInvSlots, 1 do
					if self.PlayerData.items[i] == nil then
						...
						TriggerEvent("qb-log:server:CreateLog", "playerinventory", "AddItem", "green", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** get item: [slot:" ..i.."], itemname: " .. self.PlayerData.items[i].name .. ", added amount: " .. amount ..", new total amount: ".. self.PlayerData.items[i].amount)
						--TriggerClientEvent('QBCore:Notify', self.PlayerData.source, itemInfo["label"].. " toegevoegd!", "success")
						return true
					end
				end
